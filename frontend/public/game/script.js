import { assignAchievement } from './api.js';
    ...
    try {
        ...
        if (Math.floor(playerX) === targetX && Math.floor(playerY) === targetY) {
            Swal.fire({
                ...
                if (token) {
                    assignAchievement(token, 'Пройти змейку')
                        .then(() => {
                            console.log('Достижение успешно выдано');
                        })
                        .catch((error) => {
                            if (error.response && error.response.status === 400) {
                                console.log('Достижение уже было выдано ранее.');
                            } else {
                                console.error('Ошибка при выдаче достижения:', error);
                            }
                        });
                } else {
                    console.warn('Токен пользователя отсутствует, достижение не может быть выдано.');
                }
            });
        } else {
